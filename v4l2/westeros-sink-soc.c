...
{
   if (WST_SINK_MODE_RAW != sink->sinkMode)
   {
      GstVideoAFDMeta* afd;
      GstVideoBarMeta* bar;
...
{
   //Follow previous approach - Move all these to Ready_to_Paused state
   gboolean result= FALSE;
...
gboolean gst_westeros_sink_soc_paused_to_ready( GstWesterosSink *sink, gboolean *passToDefault )
{
   ...
   {
      gst_westeros_sink_raw_playing_to_paused(sink, passToDefault);
   }
...
gboolean gst_westeros_sink_soc_accept_caps( GstWesterosSink *sink, GstCaps *caps )
{
   ...
   if( structure )
   {
      ...
      if ( mime )
      {
         ...
         GST_DEBUG("SinkMode detected as %s", (sink->sinkMode == WST_SINK_MODE_RAW) ? "RAW" : "ENCODED");
      ...
      if ( result == TRUE )
      {
         ...
         {
            /* As - The Logic here is, we are starting Encoded Resource without waiting for CAPS. */
            /* So in caseof RAW, we need to teardown the Encoded Resources First before starting RAW. */
            /* As it has to be done only once, and Not for All Caps Event, we are handling it with Flag */
            if (!sink->isSOCTearDownDone)
            {
               wstStopEvents( sink );
               wstTearDownInputBuffers( sink );
               wstTearDownOutputBuffers( sink );
               sink->isSOCTearDownDone = TRUE;
            }
            ...
            /*As now Raw Caps, we need to handle DRM Init and SinkAcquireResource */
            if (result)
            {
               gboolean passToDefault = TRUE;
               result = gst_westeros_sink_raw_resource_init( sink, &passToDefault );
            }
            //check the videoclientconnection - if connected just check for parameter changes any is theere
            if (!sink->soc.conn)
            {
               gboolean passToDefault = TRUE;
               result = gst_westeros_sink_raw_video_client_connection( sink, &passToDefault );
            }
         }
      }
   }
...
static bool wstSendFrameVideoClientConnection( WstVideoClientConnection *conn, int buffIndex )
{
   ...
   if (WST_SINK_MODE_RAW != conn->sink->sinkMode)
   {
      GstWesterosSink *sink= conn->sink;
      int sentLen;
   ...
   for( ; ; )
   {
      ...
      else if ( sink->soc.videoPaused && !sink->soc.frameAdvance )
      {
         ...
         if ( sink->soc.hasEvents )
         {
            ...
            if ( (!sink->soc.numBuffersOut || (sink->soc.decoderLastFrame || sink->soc.expectNoLastFrame)) && (havePriEvent || (pfd.revents & POLLPRI)) )
            {
               havePriEvent= false;
...
static int sinkAcquireVideo( GstWesterosSink *sink )
{
   ...
   wstGetInputFormats( sink ); //teardown GetInputFormats in soc_accept_caps on raw mode

   wstGetOutputFormats( sink ); //teardown GetInputFormats in soc_accept_caps on raw mode

   wstGetMaxFrameSize( sink ); //teardown GetInputFormats in soc_accept_caps on raw mode
