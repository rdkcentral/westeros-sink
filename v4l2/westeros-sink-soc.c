...
static void avProgInit()
{
   ...
   if ( env )
   {
      ...
      {
         /* Security fix: Use only a fixed filename, never construct path from user input.
          * This completely eliminates path injection risk. The AV_PROGRESSION env var
          * is only used to gate whether logging is enabled at all. */
         int fd;

         fd= open( "/tmp/avprog.log",
                   (O_CREAT|O_APPEND|O_WRONLY|O_CLOEXEC|O_NOFOLLOW),
                   (S_IRUSR|S_IWUSR) );
         if ( fd >= 0 )
         {
            gAvProgOut= fdopen( fd, "a" );
            if ( !gAvProgOut )
            {
               close(fd);
               GST_WARNING("avProgInit: failed to associate stream with log file: /tmp/avprog.log");
            }
         }
         else
         {
            GST_WARNING("avProgInit: failed to open log file: /tmp/avprog.log (errno %d)", errno);
         }
      }
   }
}
...
gboolean gst_westeros_sink_soc_init( GstWesterosSink *sink )
{
   ...
   sink->soc.sinkMode        = WST_SINK_MODE_ENCODED;
...
WstVideoClientConnection *wstCreateVideoClientConnection( GstWesterosSink *sink, const char *name )
{
   ...
   if ( conn )
   {
      ...
      snprintf( conn->addr.sun_path, sizeof(conn->addr.sun_path), "%s/%s", workingDir, conn->name );
...
static void wstLowLatencyModeSetVideoRect(GstWesterosSink *sink)
{
   ...
   int fd = open(SYSFS_VIDEO_AXIS, O_WRONLY | O_CLOEXEC);
   if(fd < 0)
	{
   ...
   {
      char axisStr[64];
      int n = snprintf(axisStr, sizeof(axisStr), "%d %d %d %d", top_left_x, top_left_y, bottom_right_x, bottom_right_y);
      if ( (n <= 0) || (n >= (int)sizeof(axisStr)) || (write(fd, axisStr, n) != n) )
      {
         GST_ERROR("Failed to write %s, ret: %d", SYSFS_VIDEO_AXIS, errno);
      }
   }
   close(fd);
}
