...
/*void gst_westeros_sink_raw_class_init(GstWesterosSinkClass *klass)
{
   ...
   gst_westeros_sink_raw_install_properties (gobject_class);
   gst_westeros_sink_raw_register_signals (klass);
...
void gst_westeros_sink_raw_class_init(GstWesterosSinkClass *klass)
{
   GST_DEBUG("USHA: gst_westeros_sink_raw_class_init: Enters");
   GObjectClass *gobject_class = G_OBJECT_CLASS(klass);
   GstBaseSinkClass *basesink_class = GST_BASE_SINK_CLASS(klass);
   GstElementClass *element_class = GST_ELEMENT_CLASS(klass);

   /*raw-only signals and properties - already taken care at sink_class_init*/
   GST_DEBUG("USHA: gst_westeros_sink_raw_class_init: properties and signal taken care in sink_class_init itself");
   GST_DEBUG("USHA: gst_westeros_sink_raw_class_init: Exit");
}
static void gst_westeros_sink_raw_init_common (GstWesterosSink *sink)
{
   GST_DEBUG("USHA: gst_westeros_sink_raw_init_common: Enter");
   #ifdef GLIB_VERSION_2_32
   ...
   GST_DEBUG("USHA: gst_westeros_sink_raw_init_common: only Raw Param default assing done");
   GST_DEBUG("USHA: gst_westeros_sink_raw_init_common: Exit");
}

gboolean gst_westeros_sink_raw_init( GstWesterosSink *sink )
{
   GST_DEBUG("USHA: gst_westeros_sink_raw_init: Enter");
   GST_DEBUG("USHA: gst_westeros_sink_raw_init: on backend selection - only raw related param default assign");
   GST_INFO("westeros-sink-raw.c: initializing raw backend");
   gboolean result= FALSE;

   gst_westeros_sink_raw_init_common(sink);
   
   GST_DEBUG("USHA: gst_westeros_sink_raw_init: other param get assign to default param");
   const char *env;
   int rc;
   ...
   result= TRUE;
   GST_DEBUG("USHA: gst_westeros_sink_raw_init: Exit");
   return result;
}

void gst_westeros_sink_raw_term( GstWesterosSink *sink )
{
   GST_DEBUG("USHA: gst_westeros_sink_raw_term: Enters");
   if ( sink->raw.haveDrmBuffSem )
   {
   GST_DEBUG("USHA: gst_westeros_sink_raw_term: sem-destroy");
      sink->raw.haveDrmBuffSem= false;
   ...
   #endif
   GST_DEBUG("USHA: gst_westeros_sink_raw_term: Exit");
}

/* Release only the VideoClientConnection created in ready_to_paused.
 * Called by wstEncodedRawPath when CAPS confirms SOC mode, to undo the
 * Raw connection that was pre-created before CAPS was known. */
void gst_westeros_sink_raw_release_video_conn( GstWesterosSink *sink )
{
   GST_DEBUG("USHA: gst_westeros_sink_raw_release_video_conn: Enter");
   /* Do NOT call LOCK(sink) here - caller already holds the lock */
   if ( sink->raw.conn )
   {
      GST_DEBUG("USHA: gst_westeros_sink_raw_release_video_conn: destroying RAW VideoClientConnection");
   ...
   sink->raw.videoPlaying = FALSE;
   sink->raw.videoPaused  = FALSE;
   GST_DEBUG("USHA: gst_westeros_sink_raw_release_video_conn: Exit");
}
...
{
   GST_DEBUG("USHA: gst_westeros_sink_raw_registryHandleGlobal : Enters");
   WESTEROS_UNUSED(version);
   int len;

   len= strlen(interface);
   GST_DEBUG("USHA: gst_westeros_sink_raw_registryHandleGlobal : If Interface is 'wl_sb' then bind with raw.sb");
   ...
   if ( sink->raw.useCaptureOnly )
   {
      ...
      if ( sink->vpc )
      {
   GST_DEBUG("USHA: gst_westeros_sink_raw_registryHandleGlobal : Destroy BPC on capture mode");
         wl_vpc_destroy( sink->vpc );
         sink->vpc= 0;
      }
   }
   GST_DEBUG("USHA: gst_westeros_sink_raw_registryHandleGlobal : Exit");
}
...
{
   GST_DEBUG("USHA: gst_westeros_sink_raw_null_to_ready: Enters");
   gboolean result= FALSE;

   WESTEROS_UNUSED(passToDefault);

   GST_DEBUG("USHA: gst_westeros_sink_raw_null_to_ready: checking sinkAcquireResources");
   ...
   GST_DEBUG("USHA: gst_westeros_sink_raw_null_to_ready: drmInit function called");
   if ( drmInit( sink ) )
   {
      result= TRUE;
   }
...
gboolean gst_westeros_sink_raw_ready_to_paused( GstWesterosSink *sink, gboolean *passToDefault )
{
   ...
   GST_DEBUG("USHA: gst_westeros_sink_raw_ready_to_paused: Enter");
   GST_DEBUG("USHA: gst_westeros_sink_raw_ready_to_paused: Step 1: check sinkAcquireResources");
   if ( sinkAcquireResources( sink ) )
   {
      result= TRUE;
   }
   ...
   {
      GST_ERROR("gst_westeros_sink_raw_ready_to_paused: sinkAcquireResources failed");
      return FALSE;
   }

   GST_DEBUG("USHA: gst_westeros_sink_raw_ready_to_paused: Step 2: check drmInit");
   if ( !drmInit( sink ) )
   {
      GST_ERROR("gst_westeros_sink_raw_ready_to_paused: drmInit failed");
      return FALSE;
   }

   if ( !sink->raw.useCaptureOnly )
   {
      GST_DEBUG("USHA: gst_westeros_sink_raw_ready_to_paused: Step 3: on !raw.useCaptureOnly, calling wstCreateVideoClientConnection");
...
{
   GST_DEBUG("USHA: gst_westeros_sink_raw_paused_to_playing: Enter");
   WESTEROS_UNUSED(passToDefault);
   ...
   #endif
   {
      GST_DEBUG("USHA: gst_westeros_sink_raw_paused_to_playing: sink->raw.updateSession= TRUE;");
      sink->raw.updateSession= TRUE;
   }
   UNLOCK( sink );
GST_DEBUG("USHA: gst_westeros_sink_raw_paused_to_playing: calling wstSendPauseVideoClientConnection( sink->raw.conn, false );");
   wstSendPauseVideoClientConnection( sink->raw.conn, false );
GST_DEBUG("USHA: gst_westeros_sink_raw_paused_to_playing: Exit");
   return TRUE;
}

gboolean gst_westeros_sink_raw_playing_to_paused( GstWesterosSink *sink, gboolean *passToDefault )
{
   GST_DEBUG("USHA: gst_westeros_sink_raw_playing_to_paused: Enters");
   LOCK( sink );
   sink->raw.videoPlaying= FALSE;
   sink->raw.videoPaused= TRUE;
   UNLOCK( sink );

   GST_DEBUG("USHA: gst_westeros_sink_raw_playing_to_paused: calling wstSendPauseVideoClientConnection function");
   ...
   if (gst_base_sink_is_async_enabled(GST_BASE_SINK(sink)))
   {
      ...
      GST_DEBUG("USHA: gst_westeros_sink_raw_playing_to_paused: To complete transition to paused state in async_enabled mode, we need a preroll buffer pushed to the pad");
   ...
   GST_DEBUG("USHA: gst_westeros_sink_raw_playing_to_paused: Exit");
   return TRUE;
}

gboolean gst_westeros_sink_raw_paused_to_ready( GstWesterosSink *sink, gboolean *passToDefault )
{
   GST_DEBUG("USHA: gst_westeros_sink_raw_paused_to_ready : Enters");
   ...
   if (gst_base_sink_is_async_enabled(GST_BASE_SINK(sink)))
   {
   GST_DEBUG("USHA: gst_westeros_sink_raw_paused_to_ready : if(gst_base_sink_is_async_enabled)");
      *passToDefault= true;
   }
   else
   {
   ...
   GST_DEBUG("USHA: gst_westeros_sink_raw_paused_to_ready : Exit");
   return TRUE;
}

gboolean gst_westeros_sink_raw_ready_to_null( GstWesterosSink *sink, gboolean *passToDefault )
{
   GST_DEBUG("USHA: gst_westeros_sink_raw_ready_to_null : Enters");
   WESTEROS_UNUSED(sink);

   GST_DEBUG("USHA: gst_westeros_sink_raw_ready_to_null : calling wstSinkRawStopVideo ");   
   wstSinkRawStopVideo( sink );

   GST_DEBUG("USHA: gst_westeros_sink_raw_ready_to_null : calling drmTerm ");
   drmTerm( sink );

   *passToDefault= false;

   GST_DEBUG("USHA: gst_westeros_sink_raw_ready_to_null : Exit");
   return TRUE;
}

gboolean gst_westeros_sink_raw_accept_caps( GstWesterosSink *sink, GstCaps *caps )
{
   GST_DEBUG("USHA: gst_westeros_sink_raw_accept_caps : Enters");
   bool result= FALSE;
   ...
   if( structure )
   {
      ...
      if ( result == TRUE )
      {
         ...
         {
            GST_DEBUG("USHA: gst_westeros_sink_raw_accept_caps : got framerate num %d denom %d", num, denom);
            if ( denom == 0 ) denom= 1;
            sink->raw.frameRate= (double)num/(double)denom;
            if ( sink->raw.frameRate <= 0.0 )
            {
               GST_ERROR("gst_westeros_sink_raw_accept_caps: framerate has zero or negative value, not supported\n");
            ...
            {
               GST_DEBUG("USHA: gst_westeros_sink_raw_accept_caps : framerate changed, num %d denom %d", num, denom);
   ...
   GST_DEBUG("USHA: gst_westeros_sink_raw_accept_caps : Exits");
   return result;
}

void gst_westeros_sink_raw_set_startPTS( GstWesterosSink *sink, gint64 pts )
{
   GST_DEBUG("USHA: gst_westeros_sink_raw_set_startPTS : Enters");
   WESTEROS_UNUSED(sink);
   WESTEROS_UNUSED(pts);
   GST_DEBUG("USHA: gst_westeros_sink_raw_set_startPTS : EXits");
}

void gst_westeros_sink_raw_render( GstWesterosSink *sink, GstBuffer *buffer )
{
   GST_DEBUG("USHA: gst_westeros_sink_raw_render: Enters");
   ...
   GST_DEBUG("USHA: gst_westeros_sink_raw_render: calling gst_buffer_peek_memory to check if buffer is dma-buf");
   ...
   if ( sink->display )
   {
   GST_DEBUG("USHA: gst_westeros_sink_raw_render: have display, checking dispatch thread");
      if ( sink->raw.dispatchThread == NULL )
      {
         sink->raw.quitDispatchThread= FALSE;
         GST_DEBUG_OBJECT(sink, "starting westeros_sink_dispatch thread");
         GST_DEBUG("USHA: gst_westeros_sink_raw_render: calling wstDispatchThread to start dispatch thread");
   ...
   {
      GST_DEBUG("USHA: gst_westeros_sink_raw_render: starting eos detection thread");
      sink->raw.videoPlaying= TRUE;
      sink->raw.quitEOSDetectionThread= FALSE;
      GST_DEBUG_OBJECT(sink, "starting westeros_sink_eos thread");
      GST_DEBUG("USHA: gst_westeros_sink_raw_render: calling wstEOSDetectionThread to start eos detection thread");
      sink->raw.eosDetectionThread= g_thread_new("westerossinkEOS", wstEOSDetectionThread, sink);
   }

   GST_DEBUG("USHA: gst_westeros_sink_raw_render: GST_BASE_SINK_PREROLL_UNLOCK(GST_BASE_SINK(sink));");
   GST_BASE_SINK_PREROLL_UNLOCK(GST_BASE_SINK(sink));
   while ( sink->raw.videoPaused )
   {
      GST_DEBUG("USHA: gst_westeros_sink_raw_render: video paused, waiting...");
      ...
      if ( conn )
      {
         GST_DEBUG("USHA: gst_westeros_sink_raw_render: processing messages for video client connection");
   ...
   GST_DEBUG("USHA: gst_westeros_sink_raw_render: video not paused, proceeding with rendering");
   GST_DEBUG("USHA: gst_westeros_sink_raw_render: GST_BASE_SINK_PREROLL_LOCK(GST_BASE_SINK(sink));");
   GST_BASE_SINK_PREROLL_LOCK(GST_BASE_SINK(sink));
   ...
   if ( sink->raw.expectDummyBuffers && !isDmaBuf )
   {
      ...
      {
         LOCK(sink);
         GST_DEBUG("USHA: gst_westeros_sink_raw_render: sink->raw.expectDummyBuffers && !isDmaBuf: !sink->raw.conn && (sink->raw.frameOutCount == 0  starting first frame thread");
      ...
      {
         GST_DEBUG("USHA: gst_westeros_sink_raw_render: sink->raw.expectDummyBuffers && !isDmaBuf: (sink->raw.frameInCount == 0) && sink->raw.captureEnabled && sink->raw.useTunnelled calling gst_westeros_sink_raw_set_video_path for first frame");
      ...
      if ( sink->raw.framesBeforeHideGfx )
      {
         ...
         {
            GST_DEBUG("USHA: gst_westeros_sink_raw_render: sink->raw.framesBeforeHideGfx: if ( --sink->raw.framesBeforeHideGfx == 0 ): framesBeforeHideGfx reached 0, hiding gfx frames calling wl_surface_attach");
      ...
      GST_DEBUG("USHA: gst_westeros_sink_raw_render: returning after handling dummy buffer");
   ...
   {
      GST_DEBUG("USHA: gst_westeros_sink_raw_render: !sink->flushStarted: handling real buffer");
      gint64 nanoTime;
      gint64 duration;
      int rc, buffIndex= -1;
      ...
      GST_DEBUG("USHA: gst_westeros_sink_raw_render: !sink->flushStarted: checking memory for dma-buf");
      mem= gst_buffer_peek_memory( buffer, 0 );
      if ( gst_is_dmabuf_memory(mem) )
      {
         GST_DEBUG("USHA: gst_westeros_sink_raw_render: !sink->flushStarted: using dma-buf for input");
         drmBuff= drmImportBuffer( sink, buffer );
         if ( drmBuff )
         {
            inSize= drmBuff->size[0] + drmBuff->size[1];
            GST_DEBUG("USHA: gst_westeros_sink_raw_render: buffer %p, len %d timestamp: %lld", buffer, inSize, GST_BUFFER_PTS(buffer) );
      ...
      {
         #ifdef USE_GST1
         GST_DEBUG("USHA: gst_westeros_sink_raw_render:  !importedBuffer: mapping buffer for read");
         ...
         GST_LOG("gst_westeros_sink_raw_render: buffer %p, len %d timestamp: %lld", buffer, inSize, GST_BUFFER_PTS(buffer) );
         GST_DEBUG("USHA: gst_westeros_sink_raw_render: !importedBuffer : buffer %p, len %d timestamp: %lld calling drmGetBuffer", buffer, inSize, GST_BUFFER_PTS(buffer) );
      ...
      if ( flushStarted )
      {
         GST_DEBUG("USHA: gst_westeros_sink_raw_render: if ( flushStarted ): flush started while processing buffer, releasing drm buffer if needed and returning");
         if ( drmBuff )
         {
            GST_DEBUG("USHA: gst_westeros_sink_raw_render: if ( flushStarted ): flush started while processing buffer, calling drmReleaseBuffer");
      ...
      else
      {
         ...
         if ( GST_BUFFER_PTS_IS_VALID(buffer) )
         {
            ...
            if ( nanoTime+duration >= sink->segment.start )
            {
               ...
               if (sink->prevPositionSegmentStart != sink->positionSegmentStart)
               {
                  ...
                  GST_DEBUG("USHA: gst_westeros_sink_raw_render: SegmentStart changed! Updating first PTS to %lld on valid buffer and nanoTime+duration >= sink->segment.start", sink->firstPTS);
         ...
         if ( inSize )
         {
            GST_DEBUG("USHA: gst_westeros_sink_raw_render: if ( inSize ): starting video session");
           if ( drmBuff )
            {
               ...
               if ( !importedBuffer )
               {
                  ...
                  #ifdef USE_GST_VIDEO
                  GST_DEBUG("USHA: gst_westeros_sink_raw_render:!importedBuffer: calling gst_buffer_get_video_meta");
               ...
               {
                  LOCK(sink);
                  GST_DEBUG("USHA: gst_westeros_sink_raw_render: !sink->raw.conn && (sink->raw.frameOutCount == 0): starting first frame thread");
                  sink->raw.firstFrameThread= g_thread_new("westerossinkFFr", wstFirstFrameThread, sink);
                  UNLOCK(sink);
               }
               ...
               if ( sink->raw.enableTextureSignal )
               {
                  ...
                  p1= 0;
                  p2= 0;
                  GST_DEBUG("USHA: gst_westeros_sink_raw_render:sink->raw.enableTextureSignal: Emit Signal");
               ...
               else if ( sink->raw.captureEnabled && sink->raw.sb && sink->show )
               {
                  ...
                  if ( binfo )
                  {
                     ...
                     GST_DEBUG("USHA: gst_westeros_sink_raw_render: sink->raw.captureEnabled && sink->raw.sb && sink->show: calling wl_sb_create_planar_buffer_fd2");
                     ...
                     if ( wlbuff )
                     {
                        GST_DEBUG("USHA: gst_westeros_sink_raw_render: sink->raw.captureEnabled && sink->raw.sb && sink->show: calling wl_buffer_add_listener: wlbuff created successfully");
                        ...
                        if ( sink->raw.framesBeforeHideVideo )
                        {
                           ...
                           {
                              GST_DEBUG("USHA: gst_westeros_sink_raw_render: sink->raw.framesBeforeHideVideo: if ( --sink->raw.framesBeforeHideVideo == 0 ): framesBeforeHideVideo reached 0, hiding video frames calling wstSendHideVideoClientConnection");
               ...
               if ( sink->raw.conn )
               {
                  ...
                  else
                  {
                     if ( sink->raw.showChanged )
                     {
                        ...
                        {
                           GST_DEBUG("USHA: gst_westeros_sink_raw_render: sink->raw.showChanged: if ( !sink->raw.captureEnabled ): show state changed, calling wstSendHideVideoClientConnection with %d", !sink->show);
                     ...
                     {
                        sink->raw.frameRateChanged= FALSE;
                        GST_DEBUG("USHA: gst_westeros_sink_raw_render: sink->raw.frameRateChanged: if ( sink->raw.frameRateChanged ): frame rate changed, calling wstSendRateVideoClientConnection");
                     ...
                     GST_DEBUG("USHA: gst_westeros_sink_raw_render: sending frame to video client connection with buffIndex %d", buffIndex);
                     ...
                     if ( sink->raw.framesBeforeHideGfx )
                     {
                        if ( --sink->raw.framesBeforeHideGfx == 0 )
                        {
                           ...
                           if ( sink->show )
                           {
                              GST_DEBUG("USHA: gst_westeros_sink_raw_render: sink->raw.framesBeforeHideGfx: if ( --sink->raw.framesBeforeHideGfx == 0 ): framesBeforeHideGfx reached 0, hiding gfx frames calling wstSendHideVideoClientConnection");
   ...
   GST_DEBUG("USHA: gst_westeros_sink_raw_render: exiting");
}

void gst_westeros_sink_raw_flush( GstWesterosSink *sink )
{
   GST_DEBUG("USHA: gst_westeros_sink_raw_flush: Enter");
   if ( sink->videoStarted )
   {
      LOCK(sink);
      sink->videoStarted= FALSE;
      UNLOCK(sink);
      GST_DEBUG("USHA: gst_westeros_sink_raw_flush: calling wstSendFlushVideoClientConnection to flush video client connection");
   ...
   #endif
   UNLOCK(sink);
   GST_DEBUG("USHA: gst_westeros_sink_raw_flush: EXit");
}

gboolean gst_westeros_sink_raw_start_video( GstWesterosSink *sink )
{
   GST_DEBUG("USHA: gst_westeros_sink_raw_start_video: Enter");
   GST_DEBUG("USHA: gst_westeros_sink_raw_start_video: Nothing Done");
   WESTEROS_UNUSED(sink);
   GST_DEBUG("USHA: gst_westeros_sink_raw_start_video: Exit");
   return TRUE;
}

void gst_westeros_sink_raw_eos_event( GstWesterosSink *sink )
{
   GST_DEBUG("USHA: gst_westeros_sink_raw_eos_event: Enter");
   GST_DEBUG("USHA: gst_westeros_sink_raw_eos_event: Nothing done");
   WESTEROS_UNUSED(sink);
   GST_DEBUG("USHA: gst_westeros_sink_raw_eos_event: Exit");
}

void gst_westeros_sink_raw_set_video_path( GstWesterosSink *sink, bool useGfxPath )
{
   GST_DEBUG("USHA: gst_westeros_sink_raw_set_video_path: Enter: useGfxPath %d", useGfxPath);
   ...
   if ( sink->raw.useTunnelled )
   {
      ...
      if ( pad )
      {
         ...
         if ( structure )
         {
            ...
            GST_DEBUG("USHA: gst_westeros_sink_raw_set_video_path: push westeros-raw-rectangle");
         ...
         if ( structure )
         {
            ...
            GST_DEBUG("USHA: gst_westeros_sink_raw_set_video_path: push westeros-raw-path");
            gst_pad_push_event( pad, gst_event_new_custom(GST_EVENT_CUSTOM_UPSTREAM, structure));
         }
      }
   }
   if ( needBounds(sink) && sink->vpcSurface )
   {
      ...
      sink->raw.videoWidth= sink->windowWidth;
      sink->raw.videoHeight= sink->windowHeight;
      GST_DEBUG("USHA: gst_westeros_sink_raw_set_video_path: needBounds(sink) && sink->vpcSurface: calling  wstGetVideoBounds to get video bounds for vpcSurface");
   ...
   GST_DEBUG("USHA: gst_westeros_sink_raw_set_video_path: Exit");
}

void gst_westeros_sink_raw_update_video_position( GstWesterosSink *sink )
{
   GST_DEBUG("USHA: gst_westeros_sink_raw_update_video_position: Enter");
   ...
   GST_DEBUG("USHA: gst_westeros_sink_raw_update_video_position: calling wl_sb_create_buffer to create buffer for updating video position");
      ...
      {
         GST_DEBUG("USHA: gst_westeros_sink_raw_update_video_position: sink->raw.videoPaused: calling wstSendRectVideoClientConnection to update video position on video client connection");
      ...
      if ( sink->raw.useTunnelled )
      {
         ...
         if ( pad )
         {
            ...
            {
               GST_DEBUG("push westeros-raw-rectangle");
               GST_DEBUG("USHA: gst_westeros_sink_raw_update_video_position: push westeros-raw-rectangle");
               gst_pad_push_event( pad, gst_event_new_custom(GST_EVENT_CUSTOM_UPSTREAM, structure));
            }
         }
      }
   }
   GST_DEBUG("USHA: gst_westeros_sink_raw_update_video_position: Exit");

}

gboolean gst_westeros_sink_raw_query( GstWesterosSink *sink, GstQuery *query )
{
   GST_DEBUG("USHA: gst_westeros_sink_raw_query: Enter");
   GST_DEBUG("USHA: gst_westeros_sink_raw_query: Nothing Just return False");
   return FALSE;
}

static void wstSinkRawStopVideo( GstWesterosSink *sink )
{
   GST_DEBUG("USHA: wstSinkRawStopVideo: Enters ");
   LOCK(sink);
   sink->videoStarted= FALSE;
   GST_DEBUG("USHA: wstSinkRawStopVideo: checking if(sink->raw.conn) ");
   if ( sink->raw.conn )
   {
      GST_DEBUG("USHA: wstSinkRawStopVideo: on checking if(sink->raw.conn) calling wstDestroyVideoClientConnection");
      wstDestroyVideoClientConnection( sink->raw.conn );
      sink->raw.conn= 0;
   }
   if ( sink->raw.eosDetectionThread || sink->raw.dispatchThread )
   {
      GST_DEBUG("USHA: wstSinkRawStopVideo: on checking if(sink->raw.eosDetectionThread || sink->raw.dispatchThread) checking on Display");
      sink->raw.quitEOSDetectionThread= TRUE;
      sink->raw.quitDispatchThread= TRUE;
      if ( sink->display )
      {
      GST_DEBUG("USHA: wstSinkRawStopVideo: on checking if(sink->raw.eosDetectionThread || sink->raw.dispatchThread) call wl_display_get_fd");
   ...
   {
      sink->raw.quitEOSDetectionThread= TRUE;
      GST_DEBUG("USHA: wstSinkRawStopVideo: calling g_thread_join on eosDetectionThread");
   ...
   {
      sink->raw.quitDispatchThread= TRUE;
      GST_DEBUG("USHA: wstSinkRawStopVideo: calling g_thread_join on dispatchThread");
   ...
   GST_DEBUG("USHA: wstSinkRawStopVideo: calling drmTerm");
   drmTerm( sink );
   GST_DEBUG("USHA: wstSinkRawStopVideo: Exit");

}

static void wstGetVideoBounds( GstWesterosSink *sink, int *x, int *y, int *w, int *h )
{
   GST_DEBUG("USHA: wstGetVideoBounds: Enter");
   int vx, vy, vw, vh;
   ...
   if ( sink->raw.pixelAspectRatioChanged )
   {
      ...
      {
      GST_DEBUG("USHA: wstGetVideoBounds: sink->raw.pixelAspectRatioChanged: display and vpcSurface valid, checking if ( sink->raw.captureEnabled || sink->raw.framesBeforeHideGfx )");
         if ( sink->raw.captureEnabled || sink->raw.framesBeforeHideGfx )
         {
            GST_DEBUG("USHA: wstGetVideoBounds: sink->raw.pixelAspectRatioChanged: calling wl_vpc_surface_set_geometry with calculated video bounds for vpcSurface");
            wl_vpc_surface_set_geometry( sink->vpcSurface, vx, vy, vw, vh );
         }
         else
         {
            GST_DEBUG("USHA: wstGetVideoBounds: sink->raw.pixelAspectRatioChanged: calling wl_vpc_surface_set_geometry with window geometry for vpcSurface");
   ...
   *w= vw;
   *h= vh;
   GST_DEBUG("USHA: wstGetVideoBounds: Exit");
}

static void wstSetTextureCrop( GstWesterosSink *sink, int vx, int vy, int vw, int vh )
{
   GST_DEBUG("USHA: wstSetTextureCrop: Enter");
   GST_DEBUG("USHA: wstSetTextureCrop: vx %d vy %d vw %d vh %d window(%d, %d, %d, %d) display(%dx%d)",
          ...
          (vy < 0) || (vy+vh > sink->displayHeight) ) )
   {
      ...
      GST_DEBUG("USHA: wstSetTextureCrop: calling wl_vpc_surface_set_geometry_with_crop with calculated video bounds and crop for vpcSurface");
   ...
   else
   {
      if ( sink->raw.captureEnabled || sink->raw.framesBeforeHideGfx )
      {
         GST_DEBUG("USHA: wstSetTextureCrop:if ( sink->raw.captureEnabled || sink->raw.framesBeforeHideGfx ): calling wl_vpc_surface_set_geometry");
         wl_vpc_surface_set_geometry( sink->vpcSurface, vx, vy, vw, vh );
      }
      else
      {
          GST_DEBUG("USHA: wstSetTextureCrop:else part on if ( sink->raw.captureEnabled || sink->raw.framesBeforeHideGfx ): calling wl_vpc_surface_set_geometry");
         wl_vpc_surface_set_geometry( sink->vpcSurface, sink->windowX, sink->windowY, sink->windowWidth, sink->windowHeight );
      }
   }
   GST_DEBUG("USHA: wstSetTextureCrop: Exit");
}

static WstRawVideoClientConnection *wstCreateVideoClientConnection( GstWesterosSink *sink, const char *name )
{
   GST_DEBUG("USHA: wstCreateVideoClientConnection: Enters");
   ...
   GST_DEBUG("USHA: wstCreateVideoClientConnection: creating WstRawVideoClientConnection (conn) ");
   ...
   if ( conn )
   {
      ...
      conn->name= name;
      conn->sink= sink;
   GST_DEBUG("USHA: wstCreateVideoClientConnection: con->name: %s",conn->name);
      #ifdef GLIB_VERSION_2_32
      ...
      GST_ERROR("USHA: wstCreateVideoClientConnection: socketFd: %d", conn->socketFd );
      ...
      GST_ERROR("USHA: wstCreateVideoClientConnection: calling  wstSendResourceVideoClientConnection" );
      wstSendResourceVideoClientConnection( conn );

      error= false;
   }
   GST_DEBUG("USHA: wstCreateVideoClientConnection: Exit");
exit:
...
static void wstDestroyVideoClientConnection( WstRawVideoClientConnection *conn )
{
   GST_DEBUG("USHA: wstDestroyVideoClientConnection: Enters");
   if ( conn )
   {
      GST_DEBUG("USHA: wstDestroyVideoClientConnection: on conn");
      LOCK_CONN(conn);
      conn->addr.sun_path[0]= '\0';

      if ( conn->socketFd >= 0 )
      {
         GST_DEBUG("USHA: wstDestroyVideoClientConnection: on conn close socket FD");
         close( conn->socketFd );
         conn->socketFd= -1;
      }
      UNLOCK_CONN(conn);
      ...
      #endif
   GST_DEBUG("USHA: wstDestroyVideoClientConnection: Exit");
      free( conn );
   }
}
...
static void wstSendResourceVideoClientConnection( WstRawVideoClientConnection *conn )
{
   GST_DEBUG("USHA: wstSendResourceVideoClientConnection: Enters");
   if ( conn )
   {
   ...
   GST_DEBUG("USHA: wstSendResourceVideoClientConnection: Exit");
}

static void wstSendHideVideoClientConnection( WstRawVideoClientConnection *conn, bool hide )
{
   GST_DEBUG("USHA: wstSendHideVideoClientConnection: Enters");
   if ( conn )
   {
      struct msghdr msg;
      struct iovec iov[1];
   ...
   GST_DEBUG("USHA: wstSendHideVideoClientConnection: Exit");
}

static void wstSendSessionInfoVideoClientConnection( WstRawVideoClientConnection *conn )
{
   GST_DEBUG("USHA: wstSendSessionInfoVideoClientConnection: Enters");
   if ( conn )
   {
      GstWesterosSink *sink= conn->sink;
      struct msghdr msg;
      ...
      if ( sentLen == len )
      {
         GST_DEBUG("USHA: wstSendSessionInfoVideoClientConnection: sent session info: type %d sessionId %d to video server", sink->raw.syncType, sink->raw.sessionId);
...
static GstElement* wstFindAudioSink( GstWesterosSink *sink )
{
   ...
   GST_DEBUG("wstFindAudioSink: Exit");
   return audioSink;
}
#endif
...
static void wstSetSessionInfo( GstWesterosSink *sink )
{
   GST_DEBUG("USHA: wstSetSessionInfo: Enter");
   #if defined USE_AMLOGIC_MESON || defined USE_GENERIC_AVSYNC
   if ( sink->raw.conn )
   {
      ...
      else
      {
         ...
         GST_DEBUG("USHA: wstSetSessionInfo: calling wstFindAudioSink to find audio sink for session info");
      ...
      if ( audioSink )
      {
         sink->raw.syncType= 1;
         #ifdef USE_GENERIC_AVSYNC
         GST_DEBUG("USHA: wstSetSessionInfo: checking AVSync context for audio sink");
      ...
      {
         GST_DEBUG("USHA: wstSetSessionInfo: sending session info to video client connection: calling wstSendSessionInfoVideoClientConnection");
         wstSendSessionInfoVideoClientConnection( sink->raw.conn );
      }
   }
   #endif
   GST_DEBUG("USHA: wstSetSessionInfo: Exit");
}

static void wstSendFlushVideoClientConnection( WstRawVideoClientConnection *conn )
{
   GST_DEBUG("USHA: wstSendFlushVideoClientConnection: Enters");
   if ( conn )
   {
      struct msghdr msg;
      struct iovec iov[1];
   ...
   GST_DEBUG("USHA: wstSendFlushVideoClientConnection: Exit");
}

static void wstSendPauseVideoClientConnection( WstRawVideoClientConnection *conn, bool pause )
{
   GST_DEBUG("USHA: wstSendPauseVideoClientConnection: Enters");
   if ( conn )
   {
      struct msghdr msg;
      struct iovec iov[1];
   ...
   GST_DEBUG("USHA: wstSendPauseVideoClientConnection: Exit");
}

static void wstSendRectVideoClientConnection( WstRawVideoClientConnection *conn )
{
   GST_DEBUG("USHA: wstSendRectVideoClientConnection: Enters");
   if ( conn )
   {
      struct msghdr msg;
      struct iovec iov[1];
   ...
   GST_DEBUG("USHA: wstSendRectVideoClientConnection: Exit");
}

static void wstSendRateVideoClientConnection( WstRawVideoClientConnection *conn )
{
   GST_DEBUG("USHA: wstSendRateVideoClientConnection: Enters");
   if ( conn )
   {
      struct msghdr msg;
      struct iovec iov[1];
      ...
      len += putU32( &mbody[len], sink->raw.frameRateFractionDenom );
            ...
            g_print("USHA_METRIC: VSR_OUT num=%d denom=%d fps=%0.6f socketFd=%d\n",
               sink->raw.frameRateFractionNum,
               sink->raw.frameRateFractionDenom,
               sink->raw.frameRate,
               conn->socketFd);
      ...
      if ( sentLen == len )
      {
         ...
         g_print("USHA_METRIC: VSR_OUT_SENT bytes=%d num=%d denom=%d\n",
                 len,
                 sink->raw.frameRateFractionNum,
                 sink->raw.frameRateFractionDenom);
      }
   ...
   GST_DEBUG("USHA: wstSendRateVideoClientConnection: Exit");
}

static void wstProcessMessagesVideoClientConnection( WstRawVideoClientConnection *conn )
{
   GST_DEBUG("USHA: wstProcessMessagesVideoClientConnection: Enters");
   if ( conn )
   {
      GstWesterosSink *sink= conn->sink;
      struct pollfd pfd;
      ...
      if ( rc == 1 )
      {
         ...
         while ( len >= 4 )
         {
            if ( (m[0] == 'V') && (m[1] == 'S') )
            {
               GST_DEBUG("USHA: wstProcessMessagesVideoClientConnection: Received Message from video server VS");
               int mlen, id;
               mlen= m[2];
               if ( len >= (mlen+3) )
               {
                  ...
                  {
                     case 'R':
                        GST_DEBUG("USHA: wstProcessMessagesVideoClientConnection: Received Message from video server VS: R - frame rate Message");
                        if ( mlen >= 5)
                        {
                          int rate= getU32( &m[4] );
                        ...
                        break;
                     case 'B':
                        GST_DEBUG("USHA: wstProcessMessagesVideoClientConnection: Received Message from video server VS: B - buffer release Message");
                        if ( mlen >= 5)
                        {
                          int bi= getU32( &m[4] );
                        ...
                        break;
                     case 'S':
                        GST_DEBUG("USHA: wstProcessMessagesVideoClientConnection: Received Message from video server VS: S - status Message");
                        if ( mlen >= 13)
                        {
                           ...
                           if ( (frameTime != -1LL) && (sink->prevPositionSegmentStart != 0xFFFFFFFFFFFFFFFFLL) )
                           {
                              ...
                              {
                                 GST_DEBUG("USHA: wstProcessMessagesVideoClientConnection: first frame displayed: emit first-frame signal");
                        ...
                        break;
                     case 'U':
                           GST_DEBUG("USHA: wstProcessMessagesVideoClientConnection: Received Message from video server VS: U - underflow Message");
                        if ( mlen >= 9 )
                        {
                           guint64 frameTime= getS64( &m[4] );
                           ...
                           {
                              GST_DEBUG("USHA: wstProcessMessagesVideoClientConnection: underflow occurred: emit underflow signal");
                              sink->raw.emitUnderflowSignal= TRUE;
                           }
                        }
                        break;
                     case 'Z':
                        GST_DEBUG("USHA: wstProcessMessagesVideoClientConnection: Received Message from video server VS: Z - zoom mode Message");
                        if ( mlen >= 13)
                        {
                        ...
                        break;
                     case 'D':
                        GST_DEBUG("USHA: wstProcessMessagesVideoClientConnection: Received Message from video server VS: D - debug level Message");
                        if ( mlen >= 5)
                        {
         ...
         if ( sink->raw.emitFirstFrameSignal )
         {
            sink->raw.emitFirstFrameSignal= FALSE;
            LOCK(sink);
            GST_DEBUG("USHA: gst_westeros_sink_raw_render: emit first-frame signal");
            sink->raw.firstFrameThread= g_thread_new("westerossinkFFr", wstFirstFrameThread, sink);
            UNLOCK(sink);
         }
         if ( sink->raw.emitUnderflowSignal )
         {
            sink->raw.emitUnderflowSignal= FALSE;
            LOCK(sink);
            GST_DEBUG("USHA: gst_westeros_sink_raw_render: emit underflow signal");
            sink->raw.underflowThread= g_thread_new("westerossinkUF", wstUnderflowThread, sink);
            UNLOCK(sink);
         }
      }
   }
   GST_DEBUG("USHA: wstProcessMessagesVideoClientConnection: Exit");
}

static bool wstSendFrameVideoClientConnection( WstRawVideoClientConnection *conn, int buffIndex )
{
   GST_DEBUG("USHA: wstSendFrameVideoClientConnection: Enters");
   bool result= false;
   GstWesterosSink *sink= conn->sink;
   int sentLen;
   static gint64 s_lastFrameSendUs= 0;
   ...
   GST_DEBUG("USHA: wstSendFrameVideoClientConnection: calling wstProcessMessagesVideoClientConnection");
      wstProcessMessagesVideoClientConnection( conn );
      ...
      if ( buffIndex >= 0 )
      {
         ...
         /*{
            ...
            gint64 targetDeltaUs= 33333;
            if ( s_lastFrameSendUs > 0 )
            {
               g_print("USHA_METRIC: FRAME_SEND_DELTA_US=%" G_GINT64_FORMAT " frameOut=%d buffIndex=%d bufferId=%d frameTimeUs=%" G_GINT64_FORMAT "\n",
                       (nowUs - s_lastFrameSendUs),
                       conn->sink->raw.frameOutCount,
                       buffIndex,
                       bufferId,
                       sink->raw.drmBuffer[buffIndex].frameTime);
            }
            else
            {
               g_print("USHA_METRIC: FRAME_SEND_DELTA_US=NA frameOut=%d buffIndex=%d bufferId=%d frameTimeUs=%" G_GINT64_FORMAT "\n",
                       conn->sink->raw.frameOutCount,
                       buffIndex,
                       bufferId,
                       sink->raw.drmBuffer[buffIndex].frameTime);
            ...
            /* Pace outgoing frames to reduce burst sends that can trigger queue starvation. */
            /*if ( (sink->raw.frameRateFractionNum > 0) && (sink->raw.frameRateFractionDenom > 0) )
            {
               targetDeltaUs= ((gint64)sink->raw.frameRateFractionDenom * 1000000LL) /
                              (gint64)sink->raw.frameRateFractionNum;
            }
            else if ( sink->raw.frameRate > 0.0 )
            {
               targetDeltaUs= (gint64)(1000000.0 / sink->raw.frameRate);
            }

            g_print("USHA_METRIC: PACE_DEBUG frameRateFractionNum=%d frameRateFractionDenom=%d frameRate=%f targetDeltaUs=%" G_GINT64_FORMAT " s_lastFrameSendUs=%" G_GINT64_FORMAT " frameOut=%d\n",
                    sink->raw.frameRateFractionNum,
                    sink->raw.frameRateFractionDenom,
                    sink->raw.frameRate,
                    targetDeltaUs,
            ...
            if ( (s_lastFrameSendUs > 0) && (targetDeltaUs > 0) )
            {
               gint64 elapsedUs= nowUs - s_lastFrameSendUs;
               if ( elapsedUs < targetDeltaUs )
               {
                  gint64 sleepUs= targetDeltaUs - elapsedUs;
                  g_print("USHA_METRIC: FRAME_PACE_SLEEP_US=%" G_GINT64_FORMAT " targetDeltaUs=%" G_GINT64_FORMAT " elapsedUs=%" G_GINT64_FORMAT " frameOut=%d\n",
                          sleepUs,
                          targetDeltaUs,
                          elapsedUs,
                          conn->sink->raw.frameOutCount);
                  g_usleep( (gulong)sleepUs );
               }
            }
         }*/
         ...
         if ( needBounds(sink) )
         {
            GST_DEBUG("USHA: wstSendFrameVideoClientConnection: calling  wstGetVideoBounds to get video bounds for frame");
            wstGetVideoBounds( sink, &vx, &vy, &vw, &vh );
         }
         ...
         {
            result= true;
            s_lastFrameSendUs= g_get_monotonic_time();
         }
         else
         {
...
exit:
   ...
   GST_DEBUG("USHA: wstSendFrameVideoClientConnection: Exit");
   return result;
}

static gpointer wstDispatchThread(gpointer data)
{
   GST_DEBUG("USHA: wstDispatchThread: enter");
   GstWesterosSink *sink= (GstWesterosSink*)data;
   if ( sink->display )
   {
   ...
   GST_DEBUG("USHA: wstDispatchThread: exit");
   return NULL;
}

static gpointer wstEOSDetectionThread(gpointer data)
{
   GST_DEBUG("USHA: wstEOSDetectionThread: enter");
   GstWesterosSink *sink= (GstWesterosSink*)data;
   ...
   while( !sink->raw.quitEOSDetectionThread )
   {
      ...
      if ( !sink->raw.quitEOSDetectionThread )
      {
         ...
         if ( sink->windowChange )
         {
            ...
            GST_DEBUG("USHA: wstEOSDetectionThread: window change detected: update video position calling gst_westeros_sink_raw_update_video_position");
         ...
         if ( eosEventSeen )
         {
            ...
            GST_DEBUG("USHA: wstEOSDetectionThread: window change detected: update video position calling wstProcessMessagesVideoClientConnection");
   ...
   GST_DEBUG("USH: wstEOSDetectionThread: exit");
...
static gpointer wstFirstFrameThread(gpointer data)
{
   GST_DEBUG("USHA:wstFirstFrameThread: enter");
   GstWesterosSink *sink= (GstWesterosSink*)data;

   if ( sink )
   {
      GST_DEBUG("USHA: wstFirstFrameThread: emit first frame signal");
   ...
   GST_DEBUG("USHA:wstFirstFrameThread: exit");
   return NULL;
}

static gpointer wstUnderflowThread(gpointer data)
{
   GST_DEBUG("USHA: wstUnderflowThread: enter");
   GstWesterosSink *sink= (GstWesterosSink*)data;

   if ( sink )
   {
      GST_DEBUG("USHA: wstUnderflowThread: emit underflow signal");
   ...
   GST_DEBUG("USHA:wstUnderflowThread: exit");
   return NULL;
}

static void wstBuildSinkCaps_raw( GstWesterosSinkClass *klass )
{
   GST_DEBUG("USHA: wstBuildSinkCaps_raw: enter");
   GstCaps *caps= 0;
   GstCaps *capsTemp= 0;
   ...
   GST_DEBUG("USHA: wstBuildSinkCaps_raw: exit");
}
...
static void drmTerm( GstWesterosSink *sink )
{
   ...
   GST_DEBUG("USHA:drmTerm: exit");
}

static bool drmAllocBuffer( GstWesterosSink *sink, int buffIndex, int width, int height )
{
   GST_DEBUG("USHA:drmAllocBuffer: enter");
   bool result= false;
   WstDrmBuffer *drmBuff= 0;
   if ( buffIndex < WST_NUM_DRM_BUFFERS )
   {
      ...
      GST_DEBUG("USHA: drmAllocBuffer: allocated buffer %d: handle0 %d fd0 %d pitch0 %d size0 %d offset0 %d", buffIndex, drmBuff->handle[0], drmBuff->fd[0], drmBuff->pitch[0], drmBuff->size[0], drmBuff->offset[0]);
      result= true;
   }
exit:
   if ( !result )
   ...
   GST_DEBUG("USHA:drmAllocBuffer: exit");
   return result;
}

static void drmFreeBuffer( GstWesterosSink *sink, int buffIndex )
{
   GST_DEBUG("USHA:drmFreeBuffer: enter");
   int i;
   WstDrmBuffer *drmBuff= 0;
   if (
   ...
   GST_DEBUG("USHA:drmFreeBuffer: exit");
}

static void drmLockBuffer( GstWesterosSink *sink, int buffIndex )
{
   GST_DEBUG("USHA:drmLockBuffer: enter");
   sink->raw.drmBuffer[buffIndex].locked= true;
   ++sink->raw.drmBuffer[buffIndex].lockCount;
   GST_DEBUG("USHA:drmLockBuffer: exit");
}

static bool drmUnlockBuffer( GstWesterosSink *sink, int buffIndex )
{
   GST_DEBUG("USHA:drmUnlockBuffer: enter");
   bool unlocked= false;
   ...
   GST_DEBUG("USHA:drmUnlockBuffer: exit");
   return unlocked;
}

static void drmUnlockAllBuffers( GstWesterosSink *sink )
{
   GST_DEBUG("USHA:drmUnlockAllBuffers: enter");
   WstDrmBuffer *drmBuff= 0;
   int buffIndex;
   ...
   GST_DEBUG("USHA:drmUnlockAllBuffers: exit");
}

#ifdef USE_GST_ALLOCATORS
static WstDrmBuffer *drmImportBuffer( GstWesterosSink *sink, GstBuffer *buffer )
{
   GST_DEBUG("USHA:drmImportBuffer: enter");
   WstDrmBuffer *drmBuff= 0;
   int buffIndex;
   int rc;
   ...
   for ( ; ; )
   {
      ...
      if ( rc )
      {
         ...
         {
            usleep( 1000 );
            GST_DEBUG("USHA:drmImportBuffer: retrying semaphore wait: calling wstProcessMessagesVideoClientConnection");
            wstProcessMessagesVideoClientConnection( sink->raw.conn );
            continue;
         }
      }
   ...
   for( buffIndex= 0; buffIndex < WST_NUM_DRM_BUFFERS; ++buffIndex )
   {
      ...
      if ( !drmBuff->locked )
      {
         ...
         GstMemory *mem;
         #ifdef USE_GST_VIDEO
         GST_DEBUG("USHA:drmImportBuffer: calling gst_buffer_get_video_meta to get video meta from buffer");
         GstVideoMeta *meta= gst_buffer_get_video_meta(buffer);
         #endif
         ...
         for( i= 0; i < imax; ++i )
         {
            ...
            GST_DEBUG("USHA:drmImportBuffer: got memory %p for plane %d", mem, i);
            if ( mem )
            {
               gsize offset;
   ...
   GST_DEBUG("USHA:drmImportBuffer: exit");
   return drmBuff;
}
#endif
...
static WstDrmBuffer *drmGetBuffer( GstWesterosSink *sink, int width, int height )
{
   GST_DEBUG("USHA:drmGetBuffer: enter");
   WstDrmBuffer *drmBuff= 0;
   int buffIndex;
   int rc;
   ...
   for ( ; ; )
   {
      ...
      if ( rc )
      {
         ...
         {
            usleep( 1000 );
            GST_DEBUG("USHA:drmGetBuffer: retrying semaphore wait: calling wstProcessMessagesVideoClientConnection");
            wstProcessMessagesVideoClientConnection( sink->raw.conn );
            continue;
         }
      }
...
exit:
   GST_DEBUG("USHA:drmGetBuffer: exit");
   return drmBuff;
}

static void drmReleaseBuffer( GstWesterosSink *sink, int buffIndex )
{
   GST_DEBUG("USHA:drmReleaseBuffer: enter");
   if ( !sink->raw.drmBuffer[buffIndex].locked )
   {
      int rc;
   ...
   GST_DEBUG("USHA:drmReleaseBuffer: exit");
}
...
static int sinkAcquireVideo( GstWesterosSink *sink )
{
   ...
   GST_DEBUG("USHA: sinkAcquireVideo: enter");

   LOCK(sink);
   GST_DEBUG("USHA: sinkAcquireVideo: set sink->raw.haveHardware= TRUE;");
   sink->raw.haveHardware= TRUE;
   UNLOCK(sink);

   result= 1;
   GST_DEBUG("USHA: sinkAcquireVideo: exit");
...
static void sinkReleaseVideo( GstWesterosSink *sink )
{
   GST_DEBUG("USHA: sinkReleaseVideo: enter");
   ...
   GST_DEBUG("USHA: sinkReleaseVideo: exit");
}

static int sinkAcquireResources( GstWesterosSink *sink )
{
   GST_DEBUG("USHA: sinkAcquireResources: Enters");
   int result= 0;

   GST_DEBUG("USHA: sinkAcquireResources: calling sinkAcquireVideo");
   result= sinkAcquireVideo( sink );
   GST_DEBUG("USHA: sinkAcquireResources: Exits");
   return result;
}

static void sinkReleaseResources( GstWesterosSink *sink )
{
   GST_DEBUG("USHA: sinkReleaseResources: Enters");
   sinkReleaseVideo( sink );
   GST_DEBUG("USHA: sinkReleaseResources: Exits");
}

static GstStructure *wstSinkGetStats( GstWesterosSink * sink )
{
   GST_DEBUG("USHA: wstSinkGetStats: enter");
   ...
   GST_DEBUG("USHA: wstSinkGetStats: exit");
}
