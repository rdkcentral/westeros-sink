...
static bool wstAuthenticateVideoClientConnection( WstVideoClientConnection *conn );
...
void gst_westeros_sink_raw_render( GstWesterosSink *sink, GstBuffer *buffer )
{
   ...
   if ( !sink->flushStarted )
   {
      ...
      if ( gst_is_dmabuf_memory(mem) )
      {
         ...
         GST_WARNING("USHA: gst_westeros_sink_raw_render: calling drmImportBuffer");
      ...
      if ( !importedBuffer )
      {
         ...
         GST_LOG("gst_westeros_sink_raw_render: buffer %p, len %d timestamp: %lld", buffer, inSize, GST_BUFFER_PTS(buffer) );
         GST_WARNING("USHA: gst_westeros_sink_raw_render: calling drmGetBuffer");
            ...
            if ( drmBuff )
            {
               ...
               if ( !importedBuffer )
               {
                  ...
                  if ( Y )
                  {
                     ...
                     if ( U && !V )
                     {
                        /* mmap UV plane (NV12/NV21) via prime fd */
                        data= (unsigned char*)mmap( NULL, drmBuff->size[1],
                                                    PROT_READ | PROT_WRITE, MAP_SHARED,
                                                    drmBuff->fd[1], 0 );
                        if ( data != MAP_FAILED )
                        {
                     ...
                     if ( U && V )
                     {
                        ...
                        GST_WARNING("USHA: gst_westeros_sink_raw_render: copying UV buffer data");
                        data= (unsigned char*)mmap( NULL, drmBuff->size[bi],
                                                    PROT_READ | PROT_WRITE, MAP_SHARED,
                                                    drmBuff->fd[bi], 0 );
                        if ( data != MAP_FAILED )
                        {
               ...
               if ( len >= (mlen+3) )
               {
                  ...
                  switch( id )
                  {
                     ...
                     case 'D':
                        ...
                        break;
                     case 'Q':
                        GST_WARNING("USHA: Got DRM Auth REPLY message from video server");
                        if ( mlen >= 9 )
                        {
...
static bool drmInit( GstWesterosSink *sink )
{
   GST_WARNING("USHA: drmInit: Enter");
   bool result= false;
   const char *drmName;
   int rc;
   ...
   GST_WARNING("USHA: drmInit: opened drm: %s",drmName);
   ...
   GST_WARNING("drmInit: Calling DRM_IOCTL_GET_MAGIC ioctl");
   GST_WARNING("USHA: drmInit: Calling DRM_IOCTL_GET_MAGIC ioctl");
   if ( rc != 0 )
   {
...
static bool drmAllocBuffer( GstWesterosSink *sink, int buffIndex, int width, int height )
{
   GST_WARNING("USHA: drmAllocBuffer Enter");
   bool result= false;
   WstDrmBuffer *drmBuff= 0;
   ...
   {
      GST_ERROR("drmAllocBuffer: buffIndex %d out of range", buffIndex);
      goto exit;
   }
   if ( !sink->soc.drmAuthenticated )
   {
      GST_ERROR("drmAllocBuffer: DRM auth not complete");
      goto exit;
   }
   if ( !conn || conn->socketFd < 0 )
   {
      GST_ERROR("drmAllocBuffer: no video server connection");
      goto exit;
   }
   GST_WARNING("USHA: drmAllocBuffer: sending message to gl via wstVideoServerConnectionThread socket id: %d ",conn->socketFd);

   GST_WARNING("USHA: drmAllocBuffer : doc.drmBuffer is copied to drmBuff");
   if (sink->soc.drmAuthenticated && conn && conn->socketFd >= 0)
   {
      if ( buffIndex < WST_NUM_DRM_BUFFERS )
      {
   ...
   {
      drmBuff = &sink->soc.drmBuffer[buffIndex];
      /* ---- SEND 'L': alloc request ----
      * Wire format: 'V','S', 13, 'L', width(4), height(4), buffIndex(4)
      * (mlen = 13 = 1 id byte + 12 body bytes) */
      {
         GST_WARNING("USHA: drmAllocBuffer : Send 'L' : alloc Request");
         unsigned char req[16];
         struct iovec  iov;
         struct msghdr msg;
         int sentLen, idx= 0;
         GST_WARNING("USHA: drmAllocBuffer: send IPC message with L - for asking DRM alloaction to gl");

      req[idx++]= 'V';
      req[idx++]= 'S';
      req[idx++]= 13;          /* body length: id(1) + width(4) + height(4) + buffIndex(4) */
      req[idx++]= 'L';         /* message id: aLloc */
      idx += putU32( &req[idx], (unsigned)width );
      idx += putU32( &req[idx], (unsigned)height );
      idx += putU32( &req[idx], (unsigned)buffIndex );
      ...
      do
      {
         GST_WARNING("USHA: drmAllocBuffer: send IPC message with L - for asking DRM alloaction to gl - sending request via msg");
         sentLen= sendmsg( conn->socketFd, &msg, MSG_NOSIGNAL );
		 GST_WARNING("USHA: drmAllocBuffer: send IPC message with L - send completed socket Id used: %d",conn->socketFd);
      }
      while ( (sentLen < 0) && (errno == EINTR) );

      if ( sentLen != idx )
      {
         GST_ERROR("drmAllocBuffer: sendmsg alloc request failed: errno %d", errno);
         goto exit;
      }
   }

   /* ---- RECV 'M': alloc response with fd0/fd1 via SCM_RIGHTS ---- */
   {
      GST_WARNING("USHA: drmAllocBuffer: Receive IPC message with M - Received Message from GL on buffer allocation via socket id : %d",conn->socketFd);
      ...
      do
      {
         GST_WARNING("USHA: drmAllocBuffer: M - Received Message from GL on buffer allocation - calling recvmsg to receive response from GL");
         len= recvmsg( conn->socketFd, &msg, 0 );
         GST_WARNING("USHA: drmAllocBuffer: M - Received Message from GL on buffer allocation - calling recvmsg to receive response from GL - received len %d",len);
      }
      while ( (len < 0) && (errno == EINTR) );

      if ( len >= 4 )
      {
         GST_WARNING("drmAllocBuffer: RX header bytes: [%02X %02X %02X %02X] chars: [%c %c %u %c] len=%d",
                     resp[0], resp[1], resp[2], resp[3],
                     resp[0], resp[1], resp[2], resp[3], len);
      }
      else
      {
         GST_WARNING("drmAllocBuffer: RX short response len=%d", len);
      }

      if ( len < 20 || resp[0] != 'V' || resp[1] != 'S' || resp[2] != 17 || resp[3] != 'M' )
      {
         GST_ERROR("drmAllocBuffer: unexpected response from westeros-gl (len=%d hdr=%02X %02X %02X %02X)",
                   len,
                   (len > 0 ? resp[0] : 0),
                   (len > 1 ? resp[1] : 0),
                   (len > 2 ? resp[2] : 0),
      ...
      GST_WARNING("USHA: drmAllocBuffer: send IPC message with M - parsing message and alloating DRM-BUF FD");
      /* Parse pitch/size from response body (bytes 4..19) */
      drmBuff->pitch[0]=  (int)getU32( &resp[4]  );
      drmBuff->size[0]=   (int)getU32( &resp[8]  );
      drmBuff->pitch[1]=  (int)getU32( &resp[12] );
      drmBuff->size[1]=   (int)getU32( &resp[16] );
      drmBuff->offset[0]= 0;
      drmBuff->offset[1]= 0;

      /* Extract fd0, fd1 from SCM_RIGHTS ancillary data */
      drmBuff->fd[0]= -1;
      drmBuff->fd[1]= -1;
      GST_WARNING("USHA: drmAllocBuffer: M - Received Message from GL on buffer allocation - parsing message and alloating DRM-BUF FD - parsing ancillary data for fd - for loop");
      for( cmsg= CMSG_FIRSTHDR( &msg ); cmsg; cmsg= CMSG_NXTHDR( &msg, cmsg ) )
      {
         GST_WARNING("USHA: drmAllocBuffer: M - Received Message from GL on buffer allocation - parsing message and alloating DRM-BUF FD - parsing ancillary data for fd - for loop - cmsg_level %d cmsg_type %d",cmsg->cmsg_level,cmsg->cmsg_type);
         if ( cmsg->cmsg_level == SOL_SOCKET &&
              cmsg->cmsg_type  == SCM_RIGHTS )
         {
            int nfds= (int)((cmsg->cmsg_len - CMSG_LEN(0)) / sizeof(int));
            int *fds= (int*)CMSG_DATA( cmsg );
            if ( nfds > 0 ) drmBuff->fd[0]= fds[0];
            if ( nfds > 1 ) drmBuff->fd[1]= fds[1];
            GST_WARNING("USHA: drmAllocBuffer: DRM-BUF FD received are : %d, %d * %d, %d",drmBuff->fd[0],drmBuff->fd[1],(nfds > 0 ? fds[0] : -1),(nfds > 1 ? fds[1] : -1));
            break;
         }
      }
      GST_WARNING("USHA: drmAllocBuffer: M - Received Message from GL on buffer allocation - parsing message and alloating DRM-BUF FD - parsing ancillary data for fd - for loop completed");

      if ( drmBuff->fd[0] < 0 )
      {
         GST_ERROR("drmAllocBuffer: no DMA-BUF fd received from westeros-gl");
         goto exit;
      }
   }
...
static void drmFreeBuffer( GstWesterosSink *sink, int buffIndex )
{
   ...
   drmBuff->width=  0;
   drmBuff->height= 0;
   GST_WARNING("USHA: drmFreeBuffer: Exit");
}
...
static WstDrmBuffer *drmGetBuffer( GstWesterosSink *sink, int width, int height )
{
   GST_WARNING("USHA: drmGetBuffer: Enter");
   WstDrmBuffer *drmBuff= 0;
   int buffIndex;
   int rc;
   ...
   for( buffIndex= 0; buffIndex < WST_NUM_DRM_BUFFERS; ++buffIndex )
   {
      ...
      if ( !drmBuff->locked )
      {
         ...
         {
            drmFreeBuffer( sink, buffIndex );
            GST_WARNING("USHA: drmGetBuffer : calling drmAllocBuffer");
